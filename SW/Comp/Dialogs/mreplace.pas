...
Begin
 ...
 if FParams<>nil then
  ...
  i:=0;
  while i<CbTextToRepl.Items.Count do
   begin
   BDummyS:=CbTextToRepl.Items.Strings[i];
   FParams.Values['ReplTextLast'+IntToStr(i)]:=BDummyS;
   inc(i);
   end;
  while i<32 do
   begin
   FParams.Values['ReplTextLast'+IntToStr(i)]:='';
   inc(i);
   end;
...
Procedure TReplText.SetParams ( AParams : TStringList );
Begin
 FParams:=AParams;
End;
...
Begin
 ...
 FOptions:=[];
 if StringToInteger(FParams.Values['FindTextOptions'],BParamsI) then FOptions:=TSynSearchOptions(BParamsI);
 ...
 while BIndex<32 do
  begin
  BDummyS:=FParams.Values['FindTextLast'+IntToStr(BIndex)];
 ...
 while BIndex<32 do
  begin
  BDummyS:=FParams.Values['ReplTextLast'+IntToStr(BIndex)];
  if BDummyS='' then break;
  CbTextToRepl.Items.Append(BDummyS);
  inc(BIndex);
  end;

 CbTextToFind.Text:=FTextToFind;
 CbTextToRepl.Text:=FParams.Values['ReplTextLast0'];
