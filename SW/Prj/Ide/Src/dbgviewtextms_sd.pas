...
Type
  TDbgViewTextMs = class(TDbgViewText)
  protected
...
Constructor TDbgViewTextMs.Create ( AOwner : TComponent );
Begin
 Inherited;
 FCpuType:='s';
End;

Destructor TDbgViewTextMs.Destroy;
Begin
 Inherited;
End;
...
Const
  CSymbols = '[]{}()<>",.?/;:+-='+#39;
  //CResWords = CRegNamesMs+' '+CCmdNamesMsd;
...
Procedure TDbgViewTextMs.ViewLine ( ABitmap : TBitmap; ATop : Integer; Const ALine : String );
Var
  BCanvas       : TCanvas;
  BTypeS,
  BAddrS,
...
Begin
 ...
 case BTypeS[2] of
   ...
   'x': begin
        ...
        else if StrInList(BTokenS,CRegNamesMs) then
         begin
         BCanvas.Font.Style:=[];
         BColor:=$000000;
         end
        else if StrInList(BTokenS,CCmdNamesMs) then
         begin
