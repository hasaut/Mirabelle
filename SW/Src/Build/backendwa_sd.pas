...
uses
  Classes, SysUtils, AsmTypes_sd, LlvmBase_sd,
  LlvmBeHelper_sd, Process, FileUtil;
...
Type
  ...
  TModuleWA = class(TModuleBeHelper)
  private
    FTmpFilename        : string;
...
Constructor TModuleWA.Create;
Begin
 Inherited;
 FSrcFileList:=TStringList.Create;
End;

Destructor TModuleWA.Destroy;
Begin
 FSrcFileList.Free;
 Inherited;
End;

Function TModuleWA.AppendProc : TLlvmProc;
Begin
 Result:=nil;
End;
...
Procedure TModuleWA.GenAsmSrc ( AAsmSrc : TStringList );
Var
  BCppCompStr   : string;
  BExecutable   : string;
  BProcess      : TProcess;
...
Begin
 ...
 if FileExists(BDstName) then
  begin
  if DeleteFile(BDstName)=FALSE then AppendError('e',0,0,'Cannot delete file '+BDstName+' [R:TModuleWA.GenAsmSrc]');
  end;

 {if FileExists(FTmpFilename) then
  begin
  if DeleteFile(FTmpFilename)=FALSE then AppendError('e',0,0,'Cannot delete file '+FTmpFilename+' [R:TModuleWA.GenAsmSrc]');
  end;

 if CopyFile(Filename,FTmpFilename,TRUE)=FALSE then
  begin
  AppendError('e',0,0,'Cannot copy file '+FTmpFilename+' [R:TModuleWA.GenAsmSrc]');
  break;
  end;}
 ...
 try
   ...
   AppendError('e',0,0,'Cannot execute external compiler [R:TModuleWA.GenAsmSrc]');
   break;
 end;
 ...
 if BOutList.Count=0 then
  begin
  AppendError('i',0,0,'File '+RelFilename(FPrjPath,Filename)+' was compiled by an external compiler with no messages [R:TModuleWA.GenAsmSrc]');  end
 else
  begin
  AppendError('i',0,0,'File '+RelFilename(FPrjPath,Filename)+' was compiled by an external compiler with following messages [R:TModuleWA.GenAsmSrc]');
 ...
 try
   ...
   AppendError('i',0,0,'Cannot read file '+BDstName+' [R:TModuleWA.GenAsmSrc]');
   break;
 end;
 ...
 try
   ...
   AppendError('e',0,0,'Cannot save file '+BAsmName+' [R:TModuleWA.GenAsmSrc]');
 end;
...
Function TModuleWA.GenTail : string;
Begin
 Result:='';
 repeat
...
Function TModuleWA.FormatAsmLine ( Const ALineData : string ) : string;
Var
  BLineData     : string;
Begin
 BLineData:=ALineData;
...
Procedure TModuleWA.ParseFile ( Const ALine : string );
Var
  BLine         : string;
  BParamA,
  BParamB       : string;
...
Procedure TModuleWA.ParseLoc ( Const ALine : string );
Var
  BLine         : string;
  BParamA,
  BParamB,
...
Procedure TModuleWA.ProcessGccAsmLine ( AAsmSrc : TStringList; Const ALineData : string );
Var
  BLineData     : string;
  BCmd          : string;
Begin
