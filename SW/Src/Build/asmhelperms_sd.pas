...
Type
  TAsmFlowLineMs = class(TAsmFlowLine)
  protected
...
Constructor TAsmFlowLineMs.Create;
Begin
 Inherited;
End;

Destructor TAsmFlowLineMs.Destroy;
Begin
 Inherited;
End;

Procedure TAsmFlowLineMs.Parse ( Const AReadS : string; Const AFileName : string; ALineIdx : Integer );
Var
  BCmdL         : string;
  BParamA       : string;
  BTextPos      : Integer;
...
Begin
 ...
 if Length(FParams)=0 then begin AppendError('e','Internal error: Params list is empty [R:TAsmFlowLineMs.Parse]'); break; end;
 ...
 if BCmdL='.seg' then
  begin
  FCmdIs:=acSegName;
  ReadParamA(' ',BParamA,BTextPos); if BParamA='' then AppendError('e',FParams[0],'Segment name is absent [R:TAsmFlowLineMs.Parse]');
 ...
 if BCmdL='.org' then
  begin
  FCmdIs:=acOrg;
  ReadParamA(' ',BParamA,BTextPos);if BParamA='' then begin AppendError('e',FParams[0],'ORG parameter is missing [R:TAsmFlowLineMs.Parse]'); break; end;
  AppendParam(BParamA,BTextPos);
  if TryStrToInt(FParams[1].Name,FAlign)=FALSE then AppendError('e',FParams[1],'Cannot convert ORG to integer [R:TAsmFlowLineMs.Parse]');
 ...
 if BCmdL='align' then
  begin
  FCmdIs:=acAlign;
  ReadParamA(' ',BParamA,BTextPos); if BParamA='' then begin AppendError('e',FParams[0],'Align parameter is missing [R:TAsmFlowLineMs.Parse]'); break; end;
  AppendParam(BParamA,BTextPos);
  if TryStrToInt(FParams[1].Name,FAlign)=FALSE then AppendError('e',FParams[1],'Cannot convert Align to integer [R:TAsmFlowLineMs.Parse]');
  ReadParamA(' ',BParamA,BTextPos); if BParamA<>'' then AppendError('e',AppendParam(BParamA,BTextPos),'Extra parameter in line '+BParamA+' [R:TAsmFlowLineMs.Parse]');
  break;
  end;
 ...
 if BCmdL='public' then
  begin
  FCmdIs:=acPublic;
  ReadParamA(' ',BParamA,BTextPos); if BParamA='' then begin AppendError('e',FParams[0],'Identifier name is missing [R:TAsmFlowLineMs.Parse]'); break; end;
  AppendParam(BParamA,BTextPos);
  ReadParamA(' ',BParamA,BTextPos); if BParamA<>'' then AppendError('e',AppendParam(BParamA,BTextPos),'Extra parameter in line '+BParamA+' [R:TAsmFlowLineMs.Parse]');
  break;
  end;
 ...
 if LowerCase(ReadParamStr(BExec))='equ' then
  ...
  ReadParamA(' ',BParamA,BTextPos); if BParamA='' then begin AppendError('e',FParams[0],'EQU parameter is missing [R:TAsmFlowLineMs.Parse]'); break; end;
  AppendParam(BParamA,BTextPos);
  ReadParamA(' ',BParamA,BTextPos); if BParamA<>'' then AppendError('e',AppendParam(BParamA,BTextPos),'Extra parameter in line '+BParamA+' [R:TAsmFlowLineMs.Parse]');
  break;
  end;
...
Procedure TAsmFlowLineMs.FixAddr ( Var ASegList : TMemSegList; Var ASeg : TMemSeg );
Begin
 ...
 case FCmdIs of
   acSegName:
     ...
     if ASeg=nil then begin AppendError('e',FParams[1],'Segment is not defined [R:TAsmFlowLineMs.FixAddr]'); break; end;
     end;
   acOrg:
     begin
...
Procedure TAsmFlowLineMs.UpdateRefs ( Const AInternList, AExternList : TAsmRefList; Var AIsAddrOor : boolean );
Var
  BRefIdx       : Integer;
  BRefA,
  BRefB         : TAsmRef;
...
Begin
 ...
 for BRefIdx:=0 to Length(FRefList)-1 do
  ...
  if BReadS<>'' then BRefA.AppendError('e','Extra parameter in line [R:TAsmFlowLineMs.UpdateRefs]');
  if (BOpcode<>'') and (BConstS='') then BRefA.AppendError('e','Constant is missing after operation code [R:TAsmFlowLineMs.UpdateRefs]');
  if BOpcode<>'' then
   begin
   if StrInList(BOpcode,'div shr shl add sub or and')=FALSE then BRefA.AppendError('e','Invalid operation '+BOpcode+' (only "div shr shl add sub or and" are allowed) [R:TAsmFlowLineMs.UpdateRefs]');
   if TryStrToInt(BConstS,BConst)=FALSE then BRefA.AppendError('e','Invalid constant '+BConstS+' [R:TAsmFlowLineMs.UpdateRefs]');
   end;
  repeat
  if BRefName='.' then BDstAddr:=BaseAddr
   ...
   if BRefB=nil then begin BRefA.AppendError('e','Reference '+BRefName+' not found [R:TAsmFlowLineMs.UpdateRefs]'); break; end;
   BDstAddr:=BRefB.ObjectAddr;
   end;
  if BOpcode='' then
  else if BOpcode='div' then
   begin
   if BConst=0 then BRefA.AppendError('e','Cannot divide by zero [R:TAsmFlowLineMs.UpdateRefs]')
  ...
  case BRefA.RefType of
    'j' : begin
          ...
          if (BJmpDelta>255) or (BJmpDelta<-255) then
           ...
           AIsAddrOor:=TRUE;
           FIsAddrOor:=TRUE;
           //BRefA.AppendError('e','Relative label out of range (jmp offset: '+IntToStr(BJmpDelta)+')[R:TAsmFlowLineMs.UpdateRefs]');
           end;
    ...
    'l' : begin
          ...
          if (BRefA.CodeBinPos+2)>Length(FCodeBin) then BRefA.AppendError('e','Internal error: update reference goes out of binary length [R:TAsmFlowLineMs.UpdateRefs]')
          else AddRefDW(FCodeBin,BRefA.CodeBinPos,Cardinal(BJmpDelta));
          end;
    'L' : begin
          BJmpDelta:=(BDstAddr-FBaseAddr) div 2;
          if (BRefA.CodeBinPos+4)>Length(FCodeBin) then BRefA.AppendError('e','Internal error: update reference goes out of binary length [R:TAsmFlowLineMs.UpdateRefs]')
          else AddRefDD(FCodeBin,BRefA.CodeBinPos,Cardinal(BJmpDelta));
          end;
    'b' : BRefA.AppendError('e','Linker cannot assign an external 32bit address to a 8bit reference [R:TAsmFlowLineMs.UpdateRefs]');
    'w' : BRefA.AppendError('e','Linker cannot assign an external 32bit address to a 16bit reference [R:TAsmFlowLineMs.UpdateRefs]');
    'd' : begin // Same as 'R' ?
          if (BRefA.CodeBinPos+4)>Length(FCodeBin) then BRefA.AppendError('e','Internal error: update reference goes out of binary length [R:TAsmFlowLineMs.UpdateRefs]')
    ...
    'R' : begin
          ...
          BRefA.AppendError('e','Internal error: invalid reference type #'+IntToStr(Ord(BRefA.RefType))+' [R:TAsmFlowLineMs.UpdateRefs]')
          end;
    end; // case
  until TRUE;
  end;
...
Function TAsmFlowLineMs.InvJmpA ( Const AJmp : string ) : string;
Begin
 Result:='jmp_invalid';
 ...
 else AppendError('e','Internal error: cannot invert JMP condition '+AJmp+' [R:TAsmFlowLineMs.InvJmp]');
End;

{ *** TAsmHelperMs *** }

Constructor TAsmHelperMs.Create;
Begin
 Inherited;
 FSymComment:=';';
 FSymMultiline:=#0;
...
Destructor TAsmHelperMs.Destroy;
Begin
 Inherited;
End;

Function TAsmHelperMs.NewFlowLine : TAsmFlowLine;
Begin
 Result:=TAsmFlowLineMs.Create;
End;

Procedure TAsmHelperMs.CompileLine ( ALine : TAsmFlowLine );
Var
  BRef          : TAsmRef;
Begin
 repeat
 case ALine.CmdIs of
   acPublic:
     ...
     if BRef<>nil then ALine.AppendError('e',ALine.Params[1],'Declaration with this name already exists [R:TAsmHelperMs.Compile]');
   ...
   acLabel:
     ...
     if BRef<>nil then ALine.AppendError('e',ALine.Params[0],'Label already exists [R:TAsmHelperMs.Compile]');
     RefListAppend(FLabelList,ALine.Params[0],ALine.LabelName);
     end;
   acEqu:
     begin
     if FEquList.Values[ALine.Params[0].Name]<>'' then ALine.AppendError('e',ALine.Params[0],'Duplicated EQU [R:TAsmHelperMs.Compile]')
...
Procedure TAsmHelperMs.Compile;
Var
  BLineCnt,
  BLineIdx      : Integer;
  BPath         : string;
Begin
 ...
 if FParser=nil then
  ...
  begin
  if FModule=nil then begin AppendErrorA('e',0,0,FSrcName,'Internal error: Parser is defined but module is not [R:TAsmHelperMs.Compile]'); break; end;
...
Procedure TAsmHelperMs.ExtractDupOpti ( ALine : TAsmFlowLine; AParam : TAsmLineParam; Var AReadS : string; Out ADupCnt : Integer );
Var
  BReadS        : string;
  BDupS         : string;
  BDupCnt       : Integer;
Begin
 ...
 if TryStrToInt(BDupS,BDupCnt)=FALSE then begin ALine.AppendError('e',AParam,'Cannot convert dup constant to integer [R:TAsmHelperMs.ExtractDupOpti]'); break; end;
 if BDupCnt>65536 then begin ALine.AppendError('e',AParam,'Dup counter is too big (65536 max) [R:TAsmHelperMs.ExtractDupOpti]'); break; end;
 if BDupCnt<1 then begin ALine.AppendError('e',AParam,'Dup counter is too small [R:TAsmHelperMs.ExtractDupOpti]'); break; end;
 ADupCnt:=BDupCnt;
 until TRUE;
End;

Procedure TAsmHelperMs.CodeGenDB ( ALine : TAsmFlowLine );
Var
  BParamIdx     : Integer;
...
Begin
 ...
 for BParamIdx:=1 to Length(ALine.Params)-1 do
  ...
  if (BDataS[1]=#39) and (Length(BDataS)>=2) and (BDataS[Length(BDataS)]=#39) then
   ...
   begin
   if BDataI>255 then ALine.AppendError('e',BParam,'Constant is too big for this data type [R:TAsmHelperMs.CodeGenB]')
   else if BDataI<-128 then ALine.AppendError('e',BParam,'Constant is too small for this data type [R:TAsmHelperMs.CodeGenB]');
   for BDupIdx:=1 to BDupCnt do ALine.AppendDataBinB(Byte(BDataI));
   end
  else
   begin
   ALine.AppendError('e',BParam,'Reference (if this is a reference) is too big for this data type [R:TAsmHelperMs.CodeGenB]');
   end;
  end;
End;

Procedure TAsmHelperMs.CodeGenDW ( ALine : TAsmFlowLine );
Var
  BParamIdx     : Integer;
  BParam        : TAsmLineParam;
  BDataS        : string;
...
Begin
 ...
 for BParamIdx:=1 to Length(ALine.Params)-1 do
  ...
  if (BDataS[1]=#39) and (Length(BDataS)>2) and (BDataS[Length(BDataS)]=#39) then
   ...
   begin
   if BDataI>65535 then ALine.AppendError('e',BParam,'Constant is too big for this data type [R:TAsmHelperMs.CodeGenB]')
   else if BDataI<-32768 then ALine.AppendError('e',BParam,'Constant is too small for this data type [R:TAsmHelperMs.CodeGenB]');
...
Procedure TAsmHelperMs.CodeGenDD ( ALine : TAsmFlowLine );
Var
  BParamIdx     : Integer;
  BParam        : TAsmLineParam;
  BDataS        : string;
...
Begin
 ...
 for BParamIdx:=1 to Length(ALine.Params)-1 do
  ...
  if (BDataS[1]=#39) and (Length(BDataS)>2) and (BDataS[Length(BDataS)]=#39) then
   ...
   begin
   if BDataI>$FFFFFFFF then ALine.AppendError('e',BParam,'Constant is too big for this data type [R:TAsmHelperMs.CodeGenB]')
   else if BDataI<-$80000000 then ALine.AppendError('e',BParam,'Constant is too small for this data type [R:TAsmHelperMs.CodeGenB]');
...
Function TAsmHelperMs.IsPointer ( Const AStr : string; Out ALabel : string ) : boolean;
Var
  BLen  : integer;
Begin
 Result:=FALSE;
...
Procedure TAsmHelperMs.CorrectAddrOor;
Var
  BLineIdx      : Integer;
  BLine         : TAsmFlowLine;
  BRef          : TAsmRef;
...
Begin
 ...
 while BLineIdx<Length(FFlowList) do
  ...
  if BRef=nil then begin BLine.AppendError('e','Internal error: ref is nil [R:TAsmHelperMs.CorrectAddrOor]'); break; end;
  case BRef.RefType of
   'j': begin // Becomes 'l'
        ...
        if Length(BCodeBin)<2 then begin BLine.AppendError('e','Internal error: Size of codebin is wrong [R:TAsmHelperMs.CorrectAddrOor]'); break; end;
   ...
   'l': begin // Becomes 'L'
        ...
        // ONL    |10110   ||     4cc|uuuuffff|#2/4             | bra 3xRFU
        BCodeBin:=BLine.CodeBin;
        if Length(BCodeBin)<4 then begin BLine.AppendError('e','Internal error: Size of codebin is wrong [R:TAsmHelperMs.CorrectAddrOor]'); break; end;
   ...
   'r': begin // Becomes 'R'
        ...
        BLine.AppendError('e','Internal error: Ref cannot be changed anymore [R:TAsmHelperMs.CorrectAddrOor]');
        break;
        end;
  end; // case
  until TRUE;
