...
uses
  Classes, SysUtils, LlvmBase_sd, AsmTypes_sd, ParsBase_sd, ParsHelper_sd, Process;
...
Type
  ...
  private
    ...
    Procedure GenIr ( Const AFilename : string; AIrSrc : TStringList );
  protected
...
Destructor TParsLlir.Destroy;
Begin
 Inherited;
End;

Function TParsLlir.SensCase ( Const ADataS : string ) : string;
Begin
 Result:=LowerCase(ADataS);
End;

Function TParsLlir.StrToOpcode ( Const AOpcodeS : string ) : string;
Begin
 Result:='';
...
Function TParsLlir.IsReservedWord ( Const AName : string ) : boolean;
Begin
 Result:=StrInList(SensCase(AName),CPasResWords);
End;

Function TParsLlir.IsTargConstant ( Const ANameS : string ) : string;
Var
  BNameS,
  BNameA        : string;
  BDataC        : Cardinal;
...
Procedure TParsLlir.RdTextFC ( AJmpNextLine : boolean );
Var
  BLastComment  : char;
Begin
 repeat
...
Procedure TParsLlir.RdTextFD ( AJmpNextLine : boolean );
Begin
 RdTextFC(AJmpNextLine);
End;

Procedure TParsLlir.ParseTypeA ( Out ATypeS : string; Const AKnownNames : string; AReportError : boolean );
Var
  BReadS        : string;
  BArrayS,
  BArrayE       : Integer;
...
Begin
 ...
 if SensCase(BReadS)='array' then
  begin
  if RdTextS<>'[' then AppendError('e','[ expected, \r found [R:TParsLlir.ParseTypeA]');
  ATypeS:='';
  repeat
  BReadS:=RdTextS; if TryStrToInt(BReadS,BArrayS)=FALSE then AppendError('e','Integer type expected, \r found [R:TParsLlir.ParseTypeA]');
  if RdTextS<>'..' then AppendError('e','.. expected, \r found [R:TParsLlir.ParseTypeA]');
  BReadS:=RdTextS; if TryStrToInt(BReadS,BArrayE)=FALSE then AppendError('e','Integer type expected, \r found [R:TParsLlir.ParseTypeA]');
  ATypeS:=ATypeS+'a'+IntToStr(BArrayS)+'s'+IntToStr(BArrayE)+'e';
  BReadS:=RdTextS;
  if BReadS=']' then break;
  if BReadS<>',' then begin AppendError('e','"," or "]" expected, \r found [R:TParsLlir.ParseTypeA]'); break; end;
  until FALSE;
  if ATypeS='' then ATypeS:='_';
  if RdTextS<>'of' then AppendError('e','"of" expected, \r found [R:TParsLlir.ParseTypeA]');
 ...
 if SensCase(BReadS)='string' then
  ...
  if LuTextS='[' then
   ...
   BReadS:=RdTextS; if TryStrToInt(BReadS,BArrayE)=FALSE then AppendError('e','Integer type expected, \r found [R:TParsLlir.ParseTypeA]');
   if RdTextS<>']' then AppendError('e','] expected, \r found [R:TParsLlir.ParseTypeA]');
   if BArrayE>255 then AppendError('e','String dimension is too big (max 255 allowed) [R:TParsLlir.ParseTypeA]');
   if BArrayE<=0 then AppendError('e','String dimension is too small [R:TParsLlir.ParseTypeA]');
   end
  else
   begin
   BArrayE:=255;
 ...
 if AReportError then AppendError('e','Invalid type \r [R:TParsLlir.ParseTypeA]');

 until TRUE;
End;

Function TParsLlir.ParseVar ( AVisi : char; Const AFilter : string; Const AKnownTypes : string ) : string;
Var
  BFilterS      : string;
  BReadS        : string;
  BTypeS        : string;
...
Begin
 ...
 if BReadS<>',' then begin AppendError('e',': or , expected, \r found [R:TParsLlir.ParseVar]'); SkipText(';'); break; end;
 until FALSE;
...
Function TParsLlir.ParseType ( AVisi : char; Const AKnownNames : string ) : string;
Var
  BKnownTypesS  : string;
  BReadS        : string;
  BTypeS        : string;
...
Begin
 ...
 BReadS:=RdTextS;
 BTypeName:=BReadS;
 if IsReservedWord(BReadS) then AppendError('e','Reserved word \r cannot be used as a type name [R:TParsLlir.ParseType]');
 if IsAllowedName(BReadS)=FALSE then AppendError('e','\r cannot be used as a type name [R:TParsLlir.ParseType]');
 if Pos(' '+BReadS+' ',Result)<>0 then AppendError('e','Type \r has just been declared [R:TParsLlir.ParseType]');
 if Pos(CTagM+BReadS+CTagE+' ',BKnownTypesS)<>0 then AppendError('e','Type \r is already declared [R:TParsLlir.ParseType]');
 if RdTextS<>'=' then begin AppendError('e','= expected, \r found [R:TParsLlir.ParseType]'); SkipText(';'); break; end;
...
Function TParsLlir.ResolveDirectives ( Const AName : string ) : string;
Begin
 Result:='';
 repeat
...
Function TParsLlir.ResolveConst ( Const AName : string ) : string;
Begin
 Result:='';
 repeat
...
Procedure TParsLlir.ParseConst ( AVisi : char; Const AKnownNames : string );
Var
  BKnownNamesS  : string;
  BReadS        : string;
  BConstName,
...
Begin
 ...
 if IsReservedWord(BReadS) then AppendError('e','Reserved word \r cannot be used as a Const name [R:TParsLlir.ParseConst]');
 if IsAllowedName(BReadS)=FALSE then AppendError('e','\r cannot be used as a Const name [R:TParsLlir.ParseConst]');
 if FModule.AliasExists(BConstName,TRUE) then AppendError('e','Identifier \r is already declared [R:TParsLlir.ParseConst]');
 if Pos(CTagM+BReadS+CTagE+' ',BKnownNamesS)<>0 then AppendError('e','Identifier \r is already declared [R:TParsLlir.ParseConst]');
 if RdTextS<>'=' then begin AppendError('e','= expected, \r found [R:TParsLlir.ParseConst]'); SkipText(';'); break; end;
...
Function TParsLlir.ParseProcHead ( AVisi, AProcType : char; Const AFilter : string; Const AKnownTypes : string; Out AProcName, AProcParams : string ) : string;
Var
  BFilterS      : string;
  BReadS        : string;
  BTypeS        : string;
...
Begin
 ...
 if BReadS='' then AppendError('e','Unexpected end of file')
  ...
  begin
  if AProcType='f' then AppendError('e','Function must have a return type [R:TParsLlir.ParseProcHead]');
  end
 else if BReadS=':' then
  begin
  if AProcType<>'f' then AppendError('e','Only a function can have a return type [R:TParsLlir.ParseProcHead]');
  ParseTypeA(BTypeS,AKnownTypes,TRUE);
  if RdTextS<>';' then AppendError('e','; expected \r found [R:TParsLlir.ParseProcHead]');
  end
 else
  begin
  AppendError('e','Unexpected character \r found [R:TParsLlir.ParseProcHead]');
  end;
...
Function TParsLlir.ParseProcVarTypeConst ( AProc : TLlvmProc ) : boolean;
Var
  BReadS        : string;
  BResultB      : boolean;
  BItemListS    : string;
...
Function TParsLlir.ParseProcBody ( AProc : TLlvmProc ) : boolean;
Var
  BReadS        : string;
Begin
 Result:=FALSE;
...
Procedure TParsLlir.ParseProcImpl ( AProc : TLlvmProc );
Var
  BType         : string;
Begin
 repeat
 ...
 if SensCase(LuTextS)='external' then
  ...
  if RdTextS<>';' then begin AppendError('e','; expected, \r found [R:TParsLlir.ParseProcImpl]'); break; end;
  end
 else
  begin
...
Function TParsLlir.ParseArrayOpti ( AProc : TLlvmProc; Const ATarg : string ) : string;
Var
  BReadS        : string;
  BTarg         : string;
Begin
 ...
 else AppendError('e','\r is not an array and cannot be indexed [R:TParsLlir.ParseArrayOpti]');
 RdTextS;
 Result:=Result+'[';
 repeat
 ...
 Result:=Result+BTarg;
 BReadS:=RdTextS;
 if BReadS='' then begin AppendError('e','Unexpected end of file [R:TParsLlir.ParseArrayOpti]'); break; end;
 if BReadS=']' then break;
 if BReadS<>',' then begin AppendError('e','"]" or "," expected, \r found [R:TParsLlir.ParseArrayOpti]'); break; end;
 Result:=Result+',';
 until FALSE;
 Result:=Result+']';
End;

Function TParsLlir.ParseFieldOpti ( AProc : TLlvmProc ) : string;
Var
  BReadS        : string;
  BTarg         : string;
  BField        : string;
...
Function TParsLlir.ParseEval ( AProc : TLlvmProc ) : string;
Var
  BReadS        : string;
  BOpcode       : string;
  BTarg,
...
Begin
 ...
 if BReadS='@' then
  ...
  BTarg:=ParseEval(AProc); if BTarg='' then BTarg:=CInvalidIdentifier;
  Result:=Result+BTarg;
  if RdTextS<>')' then AppendError('e',') expected, \r found [R:TParsLlir.ParseEval]');
  Result:=Result+')';
  end
 else if BReadS='[' then
  begin
  ...
  BReadS:=RdTextS;
  if BReadS=']' then break;
  if BReadS<>',' then begin AppendError('e','"," or "]" expected, \r found [R:TParsLlir.ParseEval]'); break; end;
  until FALSE;
  ...
  BTarg:='';
  if BReadS='' then begin AppendError('e','Unexpected end of file [R:ParsLlir.ParseEval]'); BTarg:=CInvalidIdentifier; end;
  if IsReservedWord(BReadS) then begin AppendError('e','Reserved word \r cannot be used as an identifier [R:ParsLlir.ParseEval]'); BTarg:=CInvalidIdentifier; end;
  if IsSymbol(BReadS[1]) then begin AppendError('e','Misplaced symbol \r [R:ParsLlir.ParseEval]'); BTarg:=CInvalidIdentifier; end;
  ...
  if BTarg='' then begin AppendError('e','Identifier \r not found [R:ParsLlir.ParseEval]'); BTarg:=CInvalidIdentifier; end;
 ...
 if BOpcode='' then begin AppendError('e','Operation code expected, \r found [R:ParsLlir.ParseEval]'); BReadS:=CInvalidOpcode; end;
 Result:=Result+' '+BOpcode+' ';
 until FALSE;
End;

Function TParsLlir.ParseCallParams ( AProc : TLlvmProc; Const AParamTypes : string ) : string;
Var
  BReadS,
  BEval         : string;
  BParams       : string;
...
Begin
 ...
 if BReadS<>'(' then
  begin
  if BParams<>'' then AppendError('e','Not enough parameters passed to the procedure/function [R:TParsLlir.ParseCallParams]');
  break;
  end;
 RdTextS;
 BReadS:=LuTextS;
  ...
  if (BEval='') and (BTypeS<>'') then AppendError('e','Not enough parameters passed to the procedure/function [R:TParsLlir.ParseCallParams]');
  if (BEval<>'') and (BTypeS='') then AppendError('e','Extra parameter in call of procedure/function [R:TParsLlir.ParseCallParams]');
  if (BEval<>'') and (BTypeS<>'') then Result:=Result+BEval+CTagP;
  if BEval<>'' then
   begin
   BReadS:=LuTextS;
   if BReadS='' then begin AppendError('e','Unexpected end of file [R:TParsLlir.ParseCallParams]'); break; end;
   if BReadS=',' then RdTextS
   else if BReadS<>')' then begin RdTextS; AppendError('e',', or ) expected, \r found [R:TParsLlir.ParseCallParams]'); end;
   end;
  until FALSE;

 BReadS:=LuTextS;
 if BReadS<>')' then AppendError('e',') expected, \r found [R:TParsLlir.ParseCallParams]')
 else RdTextS;
 until TRUE;
End;

Function TParsLlir.ParseBlock ( AProc : TLlvmProc; ALevel : Integer; Const ABreakLabel : string; Const AEndMarker : string ) : boolean;
Var
  BReadS        : string;
  BTarg,
  BEval         : string;
...
Begin
 ...
 repeat
 BReadS:=LuTextS;
 if BReadS='' then begin AppendError('e','Unexpected end of file [R:TParsLlir.ParseBlock]'); break; end;
 ...
 if SensCase(BReadS)='break' then
  begin
  RdTextS;
  if ABreakLabel='' then begin AppendError('e','Misplaced break [R:TParsLlir.ParseBlock]'); break; end;
  ...
  AppendError('e','; expected, \r found [R:TParsLlir.ParseBlock]');
  break;
  end;
 ...
 if SensCase(BReadS)='if' then
  ...
  BEval:=ParseEval(AProc); if BEval='' then begin AppendError('e','Error parsing if condition [R:TParsLlir.ParseBlock]'); BEval:=CInvalidIdentifier; end;
  AProc.FlowList.Append('if '+BEval+' '+BLThen+' '+BLElse+GenTail);
  if LuTextS<>'then' then AppendError('e','then expected, \l found [R:TParsLlir.ParseBlock]')
  else RdTextS;
  ...
  if LuTextS='begin' then // Then with begin
   ...
   if RdTextS<>'end' then begin AppendError('e','Internal error: end expected, \r found [R:TParsLlir.ParseBlock]'); break; end;
   end
  ...
  BReadS:=LuTextS;
  if (BReadS=';') and (LuTextS2='else') then AppendError('e','\l is not allowed before else [R:TParsLlir.ParseBlock]');
 ...
 if SensCase(BReadS)='while' then
  ...
  BEval:=ParseEval(AProc); if BEval='' then begin AppendError('e','Error parsing while condition [R:TParsLlir.ParseBlock]'); BEval:=CInvalidIdentifier; end;
  if Pos(#32,BEval)<>0 then BEval:='('+BEval+')';
  AProc.FlowList.Append('if '+BEval+' '+BLThen+' '+BLEnd+GenTail);
  if LuTextS<>'do' then AppendError('e','do expected, \l found [R:TParsLlir.ParseBlock]')
  else RdTextS;
  AProc.FlowList.Append(BLThen+':'+GenTail);
  if LuTextS='begin' then // while with begin
   ...
   if RdTextS<>'end' then AppendError('e','Internal error: end expected, \r found [R:TParsLlir.ParseBlock]');
   end
 ...
 if SensCase(BReadS)='repeat' then
  ...
  if RdTextS<>'until' then AppendError('e','Internal error: until expected, \r found [R:TParsLlir.ParseBlock]');
  BEval:=ParseEval(AProc); if BEval='' then begin AppendError('e','Error parsing repeat condition [R:TParsLlir.ParseBlock]'); BEval:=CInvalidIdentifier; end;
  if Pos(#32,BEval)<>0 then BEval:='('+BEval+')';
 ...
 if SensCase(BReadS)='for' then
  ...
  BReadS:=RdTextS;
  if IsReservedWord(BReadS) then AppendError('e','Misplaced \r [R:TParsLlir.ParseBlock]');
  BTarg:=GetTarg(AProc,BReadS);
  if BTarg='' then begin AppendError('e','Identifier \r not found [R:TParsLlir.ParseBlock]'); BTarg:=CInvalidIdentifier; end;
  if LuTextS<>':=' then AppendError('e',':= expected, \l found [R:TParsLlir.ParseBlock]')
  else RdTextS;
  BEval:=ParseEval(AProc); if BEval='' then begin AppendError('e','Error parsing eval [R:TParsLlir.ParseBlock]'); BEval:=CInvalidIdentifier; end;
  AProc.Flowlist.Append(BTarg+' := '+BEval+GenTail);
  BReadS:=LuTextS;
  if BReadS<>'to' then AppendError('e','to expected \l found [R:TParsLlir.ParseBlock]')
  else RdTextS;
  BEval:=ParseEval(AProc); if BEval='' then begin AppendError('e','Error parsing eval [R:TParsLlir.ParseBlock]'); BEval:=CInvalidIdentifier; end;
  AProc.FlowList.Append(BLCmp+':'+GenTail);
  if Pos(#32,BEval)<>0 then BEval:='('+BEval+')';
 ...
 if BReadS='case' then
  ...
  if LuTextS<>'of' then AppendError('e','"of" expected, \l found [R:TParsLlir.ParseBlock]')
  else RdTextS;
  if BEval='' then begin AppendError('e','Error parsing case selection [R:TParsLlir.ParseBlock]'); BEval:=CInvalidIdentifier; end;
  ...
  BLCmpNext:=AProc.LabelName('CaseCmp_'+IntToStr(BCaseIdx)+'_',ALevel,BLabelIndex);
  BLThenNext:=AProc.LabelName('CaseThen_'+IntToStr(BCaseIdx)+'_',ALevel,BLabelIndex);
  BEval:=ParseEval(AProc); if BEval='' then begin AppendError('e','Error parsing case selection [R:TParsLlir.ParseBlock]'); BEval:=CInvalidIdentifier; end;
  if RdTextS<>':' then begin AppendError('e','":" expected, \r found [R:TParsLlir.ParseBlock]'); break; end;
  ...
  if RdTextS<>';' then AppendError('e','Internal error: ";" expected, \r found [R:TParsLlir.ParseBlock]');
  AProc.FlowList.Append('jmp '+BLEnd+GenTail);
  BReadS:=LuTextS;
  if BReadS='end' then
   begin
  ...
  if BReadS='else' then
   ...
   if RdTextS<>';' then AppendError('e','Internal error: ";" expected, \r found [R:TParsLlir.ParseBlock]');
   if RdTextS<>'end' then begin AppendError('e','"end" expected after case else statement, \r found [R:TParsLlir.ParseBlock]'); break; end;
   BResultA:=TRUE;
   break;
   end;
  until FALSE;
 ...
 if SensCase(BReadS)='with' then
  ...
  BWithList:='';
  repeat
  BEval:=ParseEval(AProc); if BEval='' then begin AppendError('e','Error parsing Eval [R:TParsLlir.ParseBlock]'); BTarg:=CInvalidIdentifier; end;
  if ParsIsTypeRecord(AProc.ExtractFinalType(BEval))=FALSE then AppendError('e','Identifier or a field is not a record [R:TParsLlir.ParseBlock]');
  BWithList:=BWithList+BEval+' ';
  BReadS:=RdTextS;
  if BReadS='do' then break;
  if BReadS<>',' then break;
  until FALSE;
  if SensCase(BReadS)<>'do' then AppendError('e','"do" or "," expected, \r found [R:TParsLlir.ParseBlock]');
 ...
 else if IsReservedWord(BReadS) then begin RdTextS; AppendError('e','Misplaced \r [R:TParsLlir.ParseBlock]'); break; end;

 BTarg:=ParseEval(AProc);
 if BTarg='' then begin AppendError('e','Error parsing eval (see previous messages) [R:TParsLlir.ParseBlock]'); BTarg:=CInvalidIdentifier; end;

 if LuTextS=':=' then
  begin
  RdTextS;
  BEval:=ParseEval(AProc);
  if BEval='' then begin AppendError('e','Error parsing eval [R:TParsLlir.ParseBlock]'); BEval:=CInvalidIdentifier; end;
  AProc.FlowList.Append(BTarg+' := '+BEval+GenTail);
  BReadS:=LuTextS;
  if StrInList(BReadS,AEndMarker) then begin BResult:=TRUE; break; end;
  RdTextS;
  if BReadS<>';' then begin AppendError('e','; expected at the end of assignment statement (\r found) [R:TParsLlir.ParseBlock]'); break; end;
  BResult:=TRUE;
  break;
  end;
 ...
 if ParsIsProc(BTarg) then
  ...
  if StrInList(BReadS,AEndMarker) then begin BResult:=TRUE; break; end;
  RdTextS;
  if BReadS<>';' then begin AppendError('e','; expected (\r found) [R:TParsLlir.ParseBlock]'); break; end;
  BResult:=TRUE;
  break;
  end;

 // Something else
 AppendError('e','Construction is not recognized by the parser [R:TParsLlir.ParseBlock]');
 break;
...
Procedure TParsLlir.Parse ( AModule : TLlvmModule; AIsInclude : boolean );
Var
  BReadS        : string;
  BResultA,
  BResultB      : boolean;
...
Begin
 ...
 repeat
 // Unit
 if SensCase(RdTextS)<>'unit' then begin AppendError('e','Unit name is expected at the beginning of the file, (\r found) [R:TParsLlir.Parse]'); break; end;
 BReadS:=RdTextS;
 if BReadS='' then begin AppendError('e','Unit name is expected, (\r found) [R:TParsLlir.Parse]'); break; end;
 if IsAllowedName(BReadS)=FALSE then begin AppendError('e','Unit name error. (\r is not an allowed unit name) [R:TParsLlir.Parse]'); break; end;
 if (SensCase(BReadS)+'.pas')<>LowerCase(ExtractFileName(FFileName)) then begin AppendError('e','Unit name \r must be somewhat similar to a file name [R:TParsLlir.Parse]'); break; end;
 if AIsInclude=FALSE then FModule.Name:=BReadS;
 if RdTextS<>';' then begin AppendError('e','; is expected at the end of the line (\r found) [R:TParsLlir.Parse]'); break; end;
 // Interface
 if SensCase(RdTextS)<>'interface' then begin AppendError('e','Interface reserved word expected (\r found) [R:TParsLlir.Parse]'); break; end;
 ...
 repeat
 BReadS:=RdTextS;
 if BReadS='' then begin AppendError('e','Unexpected end of file [R:TParsLlir.Parse]'); break; end;

 if (SensCase(BReadS)='initialization') or (BReadS='finalization') or (BReadS='begin') or (BReadS='end') then
  begin
  if BVisi<>'f' then begin AppendError('e','Implementation section is missing [R:TParsLlir.Parse]'); break; end;
  BResultA:=TRUE;
  break;
  end;
 ...
 if SensCase(BReadS)='implementation' then
  begin
  if BVisi<>'h' then begin AppendError('e','Misplaced \r [R:TParsLlir.Parse]'); break; end;
  BVisi:='f';
  end
 ...
 // ** Uses section **
  ...
  repeat
  BReadS:=RdTextS;
  if IsAllowedName(BReadS)=FALSE then begin AppendError('e','Invalid unit name. (\r is not an allowed unit name) [R:TParsLlir.GetUsesOpti]'); break; end;
  if Assigned(FGetUses)=FALSE then begin AppendError('e','Internal error (ZParsLlir): FGetUses unassigned [R:TParsLlir.GetUsesOpti]'); break; end;
  BErrorA:='';
  BUsesItem:=FGetUses(BReadS,itPasUnit,BErrorA);
  if BUsesItem=nil then
   begin
   if BErrorA='' then BErrorA:='Cannot include uses module \r [R:TParsLlir.GetUsesOpti]';
   AppendError('e',BErrorA);
   break;
   end;
  if FModule.AppendUses(BUsesItem)=FALSE then begin AppendError('e','Module \r already exists in the list [R:TParsLlir.GetUsesOpti]'); break; end;
  BReadS:=RdTextS;
  if BReadS='' then begin AppendError('e','Unexpected end of file [R:TParsLlir.GetUsesOpti]'); break; end;
  if BReadS=';' then begin BResultB:=TRUE; break; end;
  if BReadS<>',' then begin AppendError('e','; or , expected, \r found [R:TParsLlir.GetUsesOpti]'); break; end;
  until FALSE;
  if BResultB=FALSE then break;
  end
 ...
 // ** Var section **
  ...
  if SensCase(LuTextS)='external' then
   ...
   RdTextS;
   //BReadS:=RdTextS;
   //if Length(BReadS)<3 then begin AppendError('e','External reference module name expected [R:TParsLlir.ParseProcImpl]'); break; end;
   //if (BReadS[1]<>#39) or (BReadS[Length(BReadS)]<>#39) then begin AppendError('e','External reference module name must be in quotes [R:TParsLlir.ParseProcImpl]'); end;
 ...
 // ** Methods **
  ...
  if BVisi='h' then
   ...
   if BProc=nil then
    ...
    if BNameA<>BNameB then AppendError('e','Procedure parameters of '+BProcName+' differ from previous declaration [R:TParsLlir.Parse]');
    //if BProc.NameL<>BItemListS then AppendError('e','Procedure parameters of '+BProcName+' differ from previous declaration[R:TParsLlir.Parse]');
    end;
   ParseProcImpl(BProc);
   end;
  end
  ...
  begin
  AppendError('e','Uses, Type, Var, Const, Procedure or function declaration expected (\r found) [R:TParsLlir.Parse]');
  break;
  end;
 until FALSE;
 ...
 if SensCase(BReadS)='end' then
  begin
  if RdTextS<>'.' then begin AppendError('e','. expected after the final end [R:TParsLlir.Parse]'); break; end;
  if RdTextS<>'' then begin AppendError('e','Extra text after the end of file [R:TParsLlir.Parse]'); break; end;
 ...
 AppendError('e','Unexpected feature \r or feature is not implemented [R:TParsLlir.Parse]');
 until TRUE;
...
Procedure TParsLlir.GenIr ( Const AFilename : string; AIrSrc : TStringList );
Var
  BCppCompStr   : string;
  BExecutable   : string;
  BProcess      : TProcess;
...
Begin
 ...
 repeat
 BCppCompStr:='clang -oS -S -emit-llvm %src% -o %dst%';
 ...
 BTmpFilename:=AssembleFullName(FDstPath,BName,'c');
 ...
 if FileExists(BTmpFilename) then
  begin
  if DeleteFile(BTmpFilename)=FALSE then AppendError('e',0,0,'Cannot delete file '+BTmpFilename+' [R:TModuleRV.GenAsmSrc]');
  end;

 if CopyFile(FModule.Filename,BTmpFilename,TRUE)=FALSE then
  begin
  AppendError('e',0,0,'Cannot copy file '+BTmpFilename+' [R:TModuleRV.GenAsmSrc]');
  break;
  end;
 ...
 ReplaceParam(BParam,'%src%',BTmpFilename);
 ...
 if BOutList.Count=0 then
  begin
  AppendError('i',0,0,'File '+RelFilename(FModule.PrjPath,FModule.Filename)+' was compiled by an external compiler with no messages [R:TModuleRV.GenAsmSrc]');  end
 else
  begin
  AppendError('i',0,0,'File '+RelFilename(FModule.PrjPath,FModule.Filename)+' was compiled by an external compiler with following messages [R:TModuleRV.GenAsmSrc]');
 ...
 // write final file
 FFilename:=BDstName; FSrc.Assign(BAsmSrc);
 until TRUE;
