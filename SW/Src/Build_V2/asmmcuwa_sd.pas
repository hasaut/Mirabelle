...
uses
  Classes, SysUtils, AsmBase_sd, AsmHelperWA_sd, ParsWA_sd, BackEndWA_sd;

Const
  CRegCount = 56;

Type
  TAsmMcuWA = class(TAsmHelperWA)
  private
...
Destructor TAsmMcuWA.Destroy;
Begin
 Inherited;
End;

Constructor TCppMcuWA.Create;
Begin
 Inherited;
 FModule:=TModuleWA.Create;
 FParser:=TParsWA.Create;
End;

Destructor TCppMcuWA.Destroy;
Begin
 FParser.Free;
 FModule.Free;
 Inherited;
...
Function TAsmMcuWA.GetXMem ( AParam : TAsmLineParam; Const ALabel : string; Out AAaaa : word; Out AConstSize : byte; Out AConst : Cardinal; Out ARef : string ) : boolean;
Var
...
Begin
 ...
 if BRilIndex>=2 then begin AParam.AppendError('e','Too many registers are used for addressing [R:TAsmMcuWA.GetXMem]'); break; end;
 ...
 if BConst>$FFFFFFFF then AParam.AppendError('e','Constant is too wide for memory addressing [R:TAsmMcuWA.GetXMem]');
 if BConst<-$80000000 then AParam.AppendError('e','Constant is too wide for memory addressing [R:TAsmMcuWA.GetXMem]');

 if BSumList.Count>=2 then
  begin
  AParam.AppendError('e','Too many references or some identifiers are not recognized as a constant or EQU: [R:TAsmMcuWA.GetXMem]');
  BSumIndex:=0;
  while BSumIndex<BSumList.Count do
   begin
 ...
 if (BRef<>'') or BConstAssigned then
  begin
  if BRegIdxList[1]<>-1 then begin AParam.AppendError('e','If constant or memory reference is used, only 1 register can be taken to calculate an address [R:TAsmMcuWA.GetXMem]'); break; end;
  ...
  else begin AParam.AppendError('e','Only awx, bwx, cwx, dwx, ewx, fwx, gwx, esp, ar, br, cr, dr, er, fr, gr registers can be used with an offset to address memory [R:TAsmMcuWA.GetXMem]'); break; end;
  if (BConstAssigned=FALSE) and (BRef<>'') then
  else if BConst>=32767 then AConstSize:=4
  else AConstSize:=2;
  ARef:=BRef; AConst:=BConst;
...
Function TAsmMcuWA.GetRegIndex ( Const ARegName : string ) : Integer;
Begin
 Result:=CRegCount-1;
 while Result>=0 do
  begin
...
Function TAsmMcuWA.GetRegIndex ( ALine : TAsmFlowLine; AParamIdx : Integer ) : Integer;
Begin
 if Length(ALine.Params)<=AParamIdx then Result:=-1
 else Result:=GetRegIndex(ALine.Params[AParamIdx].Name);
End;

Procedure TAsmMcuWA.CodeGenCmd ( ALine : TAsmFlowLine );
Var
...
Begin
 ...
 ALine.AppendError('e','Invalid command code or command does not exist with this list of parameters "'+ALine.Exec+'" [R:TAsmMcuWA.CodeGenCmd]');
 until TRUE;
End;

Function TAsmMcuWA.CodeGenBM_BC_BA ( ALine : TAsmFlowLine ) : TCmdCompError;
Var
  BCmdIdx,
  BIdxR         : Integer;
  BCmdW         : word;
...
Begin
 ...
 if (BConstI<-128) or (BConstI>255) then  begin ALine.AppendError('e',ALine.Params[2],'Constant is too wide for this register [R:TAsmMcuWA.CodeGenBM_BC_BA]'); Result:=cceError; break; end;
...
Function TAsmMcuWA.CodeGenARUC ( ALine : TAsmFlowLine ) : TCmdCompError;
Var
  BCmdIdx,
  BIdxR,
  BIdxU         : Integer;
...
Begin
 ...
 if Length(ALine.Params)=4 then
  ...
  if BWwR<>BWwU then break; // perhaps it is ARUI/ARUS begin ALine.AppendError('e',ALine.Params[2],'Registers must have same size [R:TAsmMcuWA.CodeGenARUC]'); Result:=cceError; break; end;
  end
 ...
 if IsIntegerEqu(BParamConst.Name,BConstI) then
  begin
  if BConstI>CMaxConst[BWwR] then begin ALine.AppendError('e',BParamConst,'Constant is too wide for this column [R:TAsmMcuWA.CodeGenAC]'); Result:=cceError; break; end;
  if BConstI<CMinConst[BWwR] then begin ALine.AppendError('e',BParamConst,'Constant is too wide for this column [R:TAsmMcuWA.CodeGenAC]'); Result:=cceError; break; end;
  BConst:=Cardinal(BConstI);
...
Function TAsmMcuWA.CodeGenARRS ( ALine : TAsmFlowLine ) : TCmdCompError;
Var
  BCmdIdx,
  BIdxR,
  BIdxU         : Integer;
...
Begin
 ...
 if BRrrr=CRegSpx then ALine.AppendError('w',ALine.Params[2],'With ESP use constant, otherwise it is not possible to calculate stack use [R:TAsmMcuWA.CodeGenARS]');
...
Function TAsmMcuWA.CodeGenAliasAX ( ALine : TAsmFlowLine ) : TCmdCompError;
Var
  BCmdIdx,
  BIdxR,
  BIdxU         : Integer;
...
Begin
 ...
 if BIdxU>=0 then
  begin
  BWwU:=BIdxU shr 4; BUuuu:=BIdxU and $F;
  if BRrrr=CRegSpx then ALine.AppendError('w',ALine.Params[2],'With ESP use constant, otherwise it is not possible to calculate stack use [R:TAsmMcuWA.CodeGenAliasAX]');
  BEe:=$0;
  if BExt='.' then
   begin
   if BWwR<>BWwU then begin ALine.AppendError('e',ALine.Params[2],'Registers have different size. Use "'+ALine.Params[0].Name+'zx" or "'+ALine.Params[0].Name+'sx" instead [R:TAsmMcuWA.CodeGenAliasAX]'); Result:=cceError; break; end;
 ...
 if BIdxR>=0 then // DST = Reg
  ...
  if IsIntegerEqu(ALine.Params[2].Name,BConstI) then
   begin
   if BConstI>CMaxConst[BWwR] then begin ALine.AppendError('e',ALine.Params[2],'Constant is too wide for this column [R:TAsmMcuWA.CodeGenAliasAX]'); Result:=cceError; break; end;
   if BConstI<CMinConst[BWwR] then begin ALine.AppendError('e',ALine.Params[2],'Constant is too wide for this column [R:TAsmMcuWA.CodeGenAliasAX]'); Result:=cceError; break; end;
   BConst:=Cardinal(BConstI);
   if BRrrr=CRegSpx then
    begin
    if BCmdIdx=0 then ALine.StackDelta:=-BConst;
   ...
   begin
   if BRrrr=CRegSpx then begin ALine.AppendError('e',ALine.Params[2],'Floating point constants are not applicable for ESP [R:TAsmMcuWA.CodeGenAliasAX]'); Result:=cceError; break; end;
   if BWwR<>$2 then begin ALine.AppendError('e',ALine.Params[2],'Floating point constants can only be used with 32-bit columns [R:TAsmMcuWA.CodeGenAliasAX]'); Result:=cceError; break; end;
   BConstFS:=BConstFD;
   BConst:=Cardinal(BConstFS);
   BConstSize:=4;
   end;
...
Function TAsmMcuWA.CodeGenMAID_MARC ( ALine : TAsmFlowLine ) : TCmdCompError;
Var
  BCmdIdx       : Integer;
  BRegIdx       : Integer;
  BAaaa         : word;
...
Begin
 ...
 if BCmdIdx<>-1 then
  ...
  else begin ALine.AppendError('e',ALine.Params[1],'Only 32-bit registers can be used [R:TAsmMcuWA.CodeGenMio]'); Result:=cceError; break; end;
  BWrRd:=BCmdIdx;
 ...
 if BPos=0 then
  ...
  else if BPos<>1 then begin ALine.AppendError('e',BPointer,'Post-decrement is not allowed (-- must go before the register name, not after) [R:TAsmMcuWA.CodeGenMio]'); Result:=cceError; break; end
  else BIncDec:=1;
  end
 else if BPos=1 then
  begin
  ALine.AppendError('e',BPointer,'Pre-increment is not allowed (++ must go after the register name, not before) [R:TAsmMcuWA.CodeGenMio]');
  Result:=cceError;
  break;
  end
 else
 ...
 if BIncDec<>-1 then // Continue with ++/--
  ...
  if BLabel='' then begin ALine.AppendError('e',BPointer,'Invalid memory address [R:TAsmMcuWA.CodeGenMio]'); Result:=cceError; break; end;
  BMemIdx:=GetRegIndex(BLabel);
  if BMemIdx=-1 then begin ALine.AppendError('e',BPointer,'Invalid memory address: %p [R:TAsmMcuWA.CodeGenMio]'); Result:=cceError; break; end;
  if BMemIdx in [32..47] then BAaaa:=BMemIdx-32
  else begin ALine.AppendError('e',BPointer,'Invalid memory address: %p [R:TAsmMcuWA.CodeGenMio]'); Result:=cceError; break; end;
  end;
 ...
 if BIncDec<>-1 then
  ...
  if BAaaa=CRegSpx then
   begin
   if BWw<>$2 then begin ALine.AppendError('e',BPointer,'Only 32-bit register is allowed with stack [R:TAsmMcuWA.CodeGenMio]'); Result:=cceError; break; end;
 ...
 if BIncDec<>-1 then begin ALine.AppendError('e',BPointer,'Internal error [R:TAsmMcuWA.CodeGenMAID_MARC]'); Result:=cceError; break; end;
...
Function TAsmMcuWA.CodeGenONL ( ALine : TAsmFlowLine ) : TCmdCompError;
Var
  BCmdIndex     : Integer;
  BAaaa         : word;
  BConstSize    : byte;
...
Begin
 ...
 if GetXMem(ALine.Params[1],ALine.Params[1].Name,BAaaa,BConstSize,BConst,BRef)=FALSE then
  begin
  ALine.AppendError('e',ALine.Params[1],'Invalid destination [R:TAsmMcuWA.CodeGenJcsW]');
  Result:=cceError;
  break;
  end;
...
// IDX    |10111   ||     cww|0iiirrrr|                 | inc dec
Function TAsmMcuWA.CodeGenIDX ( ALine : TAsmFlowLine ) : TCmdCompError;
Var
  BCmdIdx       : Integer;
  BIdxR         : Integer;
  BWwR          : Word;
...
Begin
 ...
 BConst:=0;
 if Length(ALine.Params)<2 then begin ALine.AppendError('e',ALine.Params[0],'Register name required [R:TAsmMcuWA.CodeGenIDE]'); Result:=cceError; break; end;
 if Length(ALine.Params)>3 then begin ALine.AppendError('e',ALine.Params[2],'Extra parameter in line: %p [R:TAsmMcuWA.CodeGenIDE]'); Result:=cceError; break; end;
 if Length(ALine.Params)=3 then
  begin
  if IsIntegerEqu(ALine.Params[2].Name,BConstI)=FALSE then begin ALine.AppendError('e',ALine.Params[2],'Invalid integer constant [R:TAsmMcuWA.CodeGenIDE]'); Result:=cceError; break; end;
  if (BConstI<1) or (BConstI>8) then begin ALine.AppendError('e',ALine.Params[2],'Constant out of range [R:TAsmMcuWA.CodeGenIDE]'); Result:=cceError; break; end;
...
Function TAsmMcuWA.CodeGenPEX ( ALine : TAsmFlowLine ) : TCmdCompError;
Var
  BCmdIdx       : Integer;
  BIdxR         : Integer;
  BWwR          : Word;
...
Begin
 ...
 if Length(ALine.Params)>2 then begin ALine.AppendError('e',ALine.Params[2],'Extra parameter in line: %p [R:TAsmMcuWA.CodeGenPEX]'); Result:=cceError; break; end;
 if Length(ALine.Params)<2 then begin ALine.AppendError('e',ALine.Params[0],'Register name required [R:TAsmMcuWA.CodeGenPEX]'); Result:=cceError; break; end;
...
Function TAsmMcuWA.CodeGenAF ( ALine : TAsmFlowLine ) : TCmdCompError;
Var
  BCmdIdx       : Integer;
  BIdxR         : Integer;
  BWwR          : Word;
...
Begin
 ...
 if Length(ALine.Params)<2 then begin ALine.AppendError('e',ALine.Params[0],'Register name required [R:TAsmMcuWA.CodeGenAF]'); Result:=cceError; break; end;
 if Length(ALine.Params)>2 then begin ALine.AppendError('e',ALine.Params[2],'Extra parameter in line: %p [R:TAsmMcuWA.CodeGenAF]'); Result:=cceError; break; end;

 BIdxR:=GetRegIndex(ALine.Params[1].Name);
 if BIdxR=-1 then break;
 BWwR:=BIdxR shr 4; BRrrr:=BIdxR and $F;
 if BWwR<>2 then begin ALine.AppendError('e',ALine.Params[0],'Register is not appicable to this command [R:TAsmMcuWA.CodeGenAF]'); Result:=cceError; break; end;
...
Function TAsmMcuWA.CodeGenREM ( ALine : TAsmFlowLine ) : TCmdCompError;
Var
  BCmdIdx       : Integer;
  BIdxR         : Integer;
  BWwR          : Word;
...
Begin
 ...
 repeat
 BCmdIdx:=GetStrIndex(LowerCase(ALine.Params[0].Name),'rem');
 if BCmdIdx=-1 then break;
...
Function TAsmMcuWA.CodeGenPRCS ( ALine : TAsmFlowLine ) : TCmdCompError;
Var
  BCmdIdx       : Integer;
  BConstI       : Int64;
  BConst        : Cardinal;
...
Begin
 ...
 if Length(ALine.Params)<2 then begin ALine.AppendError('e',ALine.Params[0],'Register name required [R:TAsmMcuWA.CodeGenPRCS]'); Result:=cceError; break; end;
 if Length(ALine.Params)>2 then begin ALine.AppendError('e',ALine.Params[2],'Extra parameter in line: %p [R:TAsmMcuWA.CodeGenPRCS]'); Result:=cceError; break; end;

 if BCmdIdx=1 then ALine.StackDelta:=4;

 if IsIntegerEqu(ALine.Params[1].Name,BConstI)=FALSE then BRef:=ALine.Params[1].Name;
...
Function TAsmMcuWA.CodeGenSys ( ALine : TAsmFlowLine ) : TCmdCompError;
Var
  BCmdIdx       : Integer;
Begin
 Result:=cceCheckNext;
...
Function TAsmMcuWA.CodeGenNTRE ( ALine : TAsmFlowLine ) : TCmdCompError;
Var
  BCmdIdx       : Integer;
Begin
 Result:=cceCheckNext;
...
Function TAsmMcuWA.GetPplRegIdx ( AParam : TAsmLineParam; AIncludeEip : boolean; Out ARegBitList : word; Out ARegCnt : Integer ) : boolean;
Var
  BRegList  : string;
  BRegIdx   : Integer;
  BRegName  : string;
...
Begin
 ...
 if BRegName='#' then
  ...
  if BRegIdx=-1 then begin AParam.AppendError('e','Invalid register name '+BRegName+' [R:TAsmMcuWA.GetPplRegIdx]'); break; end;
  BMask:=1 shl BRegIdx;
  if (ARegBitList and BMask)<>0 then AParam.AppendError('w','Register '+BRegName+' is already in the list [R:TAsmMcuWA.GetPplRegIdx]');
...
Function TAsmMcuWA.CodeGenPPL ( ALine : TAsmFlowLine ) : TCmdCompError;
Var
  BCmdIdx       : Integer;
  BRegCnt       : Integer;
  BRegBitList   : word;
...
Begin
 ...
 case BCmdIdx of
   0,        // popl
   1: begin  // pushl
      if Length(ALine.Params)<2 then begin ALine.AppendError('e',ALine.Params[0],'Register name list required [R:TAsmMcuWA.CodeGenPPL]'); break; end;
      if Length(ALine.Params)>2 then begin ALine.AppendError('e',ALine.Params[2],'Extra parameter in line: %p [R:TAsmMcuWA.CodeGenPPL]'); break; end;
   ...
   4: begin  // leave
      if Length(ALine.Params)<4 then begin ALine.AppendError('e',ALine.Params[0],'Not enough parameters (register list, local var size and stack clearance are expected) [R:TAsmMcuWA.CodeGenPPL]'); break; end;
      if Length(ALine.Params)>4 then begin ALine.AppendError('e',ALine.Params[4],'Extra parameter in line: %p [R:TAsmMcuWA.CodeGenPPL]'); break; end;
      if GetPplRegIdx(ALine.Params[1],TRUE,BRegBitList,BRegCnt)=FALSE then break;
      if IsIntegerEqu(ALine.Params[2].Name,BLocalVarSize)=FALSE then begin ALine.AppendError('e',ALine.Params[2],'Cannot convert local var size to integer [R:TAsmMcuWA.CodeGenPPL]'); break; end;
      if (BLocalVarSize and $3)<>0 then begin ALine.AppendError('e',ALine.Params[2],'Local var size must be aligned by 4 [R:TAsmMcuWA.CodeGenPPL]'); break; end;
      if BLocalVarSize>$3FF then begin ALine.AppendError('e',ALine.Params[2],'Local var size is too big [R:TAsmMcuWA.CodeGenPPL]'); break; end;
      if IsIntegerEqu(ALine.Params[3].Name,BStackClrSize)=FALSE then begin ALine.AppendError('e',ALine.Params[3],'Cannot convert stack clearance to integer [R:TAsmMcuWA.CodeGenPPL]'); break; end;
      if (BStackClrSize and $3)<>0 then begin ALine.AppendError('e',ALine.Params[3],'Stack clearance must be aligned by 4 [R:TAsmMcuWA.CodeGenPPL]'); break; end;
      if BStackClrSize>$3FF then begin ALine.AppendError('e',ALine.Params[3],'Stack clearance is too big [R:TAsmMcuWA.CodeGenPPL]'); break; end;
   ...
   5: begin  // enter
      if Length(ALine.Params)<3 then begin ALine.AppendError('e',ALine.Params[0],'Not enough parameters (register list, local var size and stack clearance are expected) [R:TAsmMcuWA.CodeGenPPL]'); break; end;
      if Length(ALine.Params)>3 then begin ALine.AppendError('e',ALine.Params[3],'Extra parameter in line: %p [R:TAsmMcuWA.CodeGenPPL]'); break; end;
      if GetPplRegIdx(ALine.Params[1],TRUE,BRegBitList,BRegCnt)=FALSE then break;
      if IsIntegerEqu(ALine.Params[2].Name,BLocalVarSize)=FALSE then begin ALine.AppendError('e',ALine.Params[2],'Cannot convert local var size to integer [R:TAsmMcuWA.CodeGenPPL]'); break; end;
      if (BLocalVarSize and $3)<>0 then begin ALine.AppendError('e',ALine.Params[2],'Local var size must be aligned by 4 [R:TAsmMcuWA.CodeGenPPL]'); break; end;
      if BLocalVarSize>$3FF then begin ALine.AppendError('e',ALine.Params[2],'Local var size is too big [R:TAsmMcuWA.CodeGenPPL]'); break; end;
      ...
      ALine.AppendError('e','Internal error [R:TAsmMcuWA.CodeGenPPL]');
      break;
      end;
 end;
...
Function TAsmMcuWA.CodeGenONS ( ALine : TAsmFlowLine ) : TCmdCompError;
Var
  BCmdIndex     : Integer;
  BFfff         : Word;
  BIsSwt        : boolean;
Begin
 ...
 BFfff:=Word(BCmdIndex);
 Result:=cceError;
 if Length(ALine.Params)>2 then begin ALine.AppendError('e',ALine.Params[2],'Extra parameter in line: %p [R:TAsmMcuWA.CodeGenJ]'); break; end
 else if Length(ALine.Params)<2 then begin ALine.AppendError('e',ALine.Params[0],'Jmp label required [R:TAsmMcuWA.CodeGenJ]'); break; end;
...
Function TAsmMcuWA.CodeGenFRUC ( ALine : TAsmFlowLine ) : TCmdCompError;
Var
  BCmdIdx,
  BIdxR,
  BIdxU         : Integer;
...
Begin
 ...
 if Length(ALine.Params)=4 then
  ...
  if BWwR<>BWwU then begin ALine.AppendError('e',ALine.Params[2],'Registers must have same size [R:TAsmMcuWA.CodeGenFRUC]'); Result:=cceError; break; end;
  end
 else if Length(ALine.Params)=3 then
  begin
  BParamConst:=ALine.Params[2];
 ...
 if BWwR<>2 then begin ALine.AppendError('e',ALine.Params[1],'Only 32-bit registers are allowed [R:TAsmMcuWA.CodeGenFRUC]'); Result:=cceError; break; end;
 if BWwU<>2 then begin ALine.AppendError('e',ALine.Params[2],'Only 32-bit registers are allowed [R:TAsmMcuWA.CodeGenFRUC]'); Result:=cceError; break; end;

 if IsPointer(BParamConst.Name,BLabel) then break; // This is not our problem anymore, just verify other cases
 if GetRegIndex(BParamConst.Name)>=0 then break;

 BConst:=0;
 if IsIntegerEqu(BParamConst.Name,BConstI) then begin ALine.AppendError('e',BParamConst,'Floating point parameter required [R:TAsmMcuWA.CodeGenFRUC]'); Result:=cceError; break; end;
 if IsFloatEqu(BParamConst.Name,BConstFD) then
  begin
  if BRrrr=CRegSpx then begin ALine.AppendError('e',BParamConst,'Floating point constants are not applicable for ESP [R:TAsmMcuWA.CodeGenAC]'); Result:=cceError; break; end;
  BConstFS:=BConstFD;
  BConst:=Cardinal(BConstFS);
  end
 else begin ALine.AppendError('e',BParamConst,'Floating point parameter required [R:TAsmMcuWA.CodeGenFRUC]'); Result:=cceError; break; end;
...
Function TAsmMcuWA.CodeGenFRRS ( ALine : TAsmFlowLine ) : TCmdCompError;
Var
  BCmdIdx,
  BIdxR,
  BIdxU         : Integer;
...
Begin
 ...
 BWwU:=BIdxU shr 4; BUuuu:=BIdxU and $F;
 if BWwR<>BWwU then break;
 if BWwR<>2 then begin ALine.AppendError('e',ALine.Params[1],'Floating point constants are only applicable for 32-bit registers [R:TAsmMcuWA.CodeGenFRRS]'); Result:=cceError; break; end;

 if BRrrr=CRegSpx then ALine.AppendError('w',ALine.Params[2],'With ESP use constant, otherwise it is not possible to calculate stack use [R:TAsmMcuWA.CodeGenFRRS]');
...
Function TAsmMcuWA.CodeGenBTR ( ALine : TAsmFlowLine ) : TCmdCompError;
Var
  BCmdIdx,
  BIdxR         : Integer;
  BWwR          : Word;
...
Begin
 ...
 if IsIntegerEqu(BIdxParam.Name,BConstI)=FALSE then begin BIdxParam.AppendError('e','Invalid bit index [R:TAsmMcuWA.CodeGenBTx]'); Result:=cceError; break; end;
 if (BConstI<0) or (BConstI>((1 shl BWwR)*8-1)) then begin BIdxParam.AppendError('e','Bit index out of range [R:TAsmMcuWA.CodeGenBTx]'); Result:=cceError; break; end;
...
Function TAsmMcuWA.CodeGenBTM ( ALine : TAsmFlowLine ) : TCmdCompError;
Var
  BCmdIdx       : Integer;
  BAaaa         : word;
  BLabel        : string;
...
Begin
 ...
 if IsIntegerEqu(BIdxParam.Name,BConstI)=FALSE then begin BIdxParam.AppendError('e','Invalid bit index [R:TAsmMcuWA.CodeGenBTx]'); Result:=cceError; break; end;
 if (BConstI<0) or (BConstI>7) then begin BIdxParam.AppendError('e','Bit index must be in the interval 0..7 [R:TAsmMcuWA.CodeGenBTx]'); Result:=cceError; break; end;
 BConstIdx:=Cardinal(BConstI);

 if IsPointer(BPointer.Name,BLabel)=FALSE then begin BPointer.AppendError('e','Pointer type expected [R:TAsmMcuWA.CodeGenBTx]'); Result:=cceError; break; end;

 if Pos('++',BLabel)<>0 then begin BPointer.AppendError('e','Increment is not allowed [R:TAsmMcuWA.CodeGenBTx]'); Result:=cceError; break; end;
 if Pos('--',BLabel)<>0 then begin BPointer.AppendError('e','Decrement is not allowed [R:TAsmMcuWA.CodeGenBTx]'); Result:=cceError; break; end;
...
Function TAsmMcuWA.CodeGenARUI_ARUS ( ALine : TAsmFlowLine ) : TCmdCompError;
Var
  BCmdIdx       : Integer;
  BCccc         : Word;
  BIndex        : Integer;
...
Begin
 ...
 if Length(ALine.Params)=4 then // 3 regs classic or 2 regs + #
  ...
  if BIndex=3 then
   begin
   if BIsRor then begin ALine.Params[0].AppendError('e','ROR command will only work with constant [R:TAsmMcuWA.CodeGenARUI_ARUS]'); Result:=cceError; break; end;
   if ((BWwList[0]<>BWwList[1]) or (BWwList[1]<>BWwList[2])) and (BSignExt='.') then begin ALine.Params[0].AppendError('e','Registers of different size: use prefixes ZX or SX [R:TAsmMcuWA.CodeGenARUI_ARUS]'); Result:=cceError; break; end;
  ...
  if BIsRor then begin ALine.Params[0].AppendError('e','ROR command will only work with constant [R:TAsmMcuWA.CodeGenARUI_ARUS]'); Result:=cceError; break; end;
  BConst:=0;
  if (BWwList[0]<>BWwList[1]) and (BSignExt='.') then begin ALine.Params[0].AppendError('e','Registers of different size: use prefixes ZX or SX [R:TAsmMcuWA.CodeGenARUI_ARUS]'); Result:=cceError; break; end;
  if IsIntegerEqu(ALine.Params[3].Name,BConstI)=FALSE then begin ALine.AppendError('e',ALine.Params[3],'Cannot convert constant to integer [R:TAsmMcuWA.CodeGenBBx]'); Result:=cceError; break; end;
  if (BConstI>255) or (BConstI<0) then begin ALine.AppendError('e',ALine.Params[2],'Constant is too big or too small [R:TAsmMcuWA.CodeGenBBx]'); Result:=cceError; break; end;
  BConst:=Cardinal(BConstI);
 ...
 if Length(ALine.Params)=3 then // 2 regs or 1 reg + # (then uuuu and rrrr are the same)
  ...
  BConst:=0;
  if IsIntegerEqu(ALine.Params[2].Name,BConstI)=FALSE then begin ALine.AppendError('e',ALine.Params[2],'Cannot convert constant to integer [R:TAsmMcuWA.CodeGenBBx]'); Result:=cceError; break; end;
  if (BConstI>255) or (BConstI<0) then begin ALine.AppendError('e',ALine.Params[2],'Constant is too big or too small [R:TAsmMcuWA.CodeGenBBx]'); Result:=cceError; break; end;
  BConst:=Cardinal(BConstI);
