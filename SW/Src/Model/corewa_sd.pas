...
uses
  Classes, SysUtils, Math, AsmTypes_sd, CoreBase_sd, DasmWA_sd;

Type
  TExecWA = Function : boolean of object;
  TExecWAList = array [TWaCmdCode] of TExecWA;

  TCoreWA = class(TCoreBase)
  private
...
Constructor TCoreWA.Create;
Begin
 Inherited;
 FExec:=TExecLineWA.Create;
...
Destructor TCoreWA.Destroy;
Begin
 FExec.Free;
 Inherited;
End;

Function TCoreWA.RegsAsStr : string;
Var
  BRegIdx   : Integer;
Begin
...
Function TCoreWA.RdRegs : string;
Var
  BRegIdx   : Integer;
Begin
...
Procedure TCoreWA.WrRegs ( Const ARegs : string );
Var
  BRegIdx   : Integer;
Begin
...
Function TCoreWA.TargSize ( ATargIdx : TMsTargIdx ) : byte;
Begin
 Result:=1 shl FExec.Subdec.FTarg[ATargIdx].FWw;
End;

Procedure TCoreWA.CropData ( Var AData : Cardinal; ATargIdx : TMsTargIdx );
Var
  BSize     : byte;
Begin
...
Function TCoreWA.RdReg ( ARow, ACol : byte ) : Cardinal;
Begin
 Result:=0;
 repeat
 if ARow=0 then
...
Function TCoreWA.RdReg ( ATargIdx : TMsTargIdx ) : Cardinal;
Var
  BRow,
  BCol      : byte;
Begin
...
Procedure TCoreWA.WrReg ( ARow, ACol : byte; AData : Cardinal );
Begin
 repeat
 if ARow=0 then
  begin
...
Procedure TCoreWA.WrReg ( ATargIdx : TMsTargIdx; AData : Cardinal );
Var
  BRow,
  BCol      : byte;
Begin
...
Procedure TCoreWA.WrReg ( Const ASubdec : TMsSubdec; ATargIdx : TMsTargIdx; AData : Cardinal );
Var
  BRow,
  BCol      : byte;
Begin
...
Function TCoreWA.GetSp : Cardinal;
Begin
 Result:=FRegList[8];
End;

Procedure TCoreWA.SetSp ( AData : Cardinal );
Begin
 FRegList[8]:=AData;
End;

Function TCoreWA.PushD ( AData : Cardinal ) : boolean;
Var
  BAddr     : Cardinal;
Begin
 Result:=FALSE;
 ...
 BAddr:=GetSp-4;
 SetSp(BAddr);
 if MioWrX(BAddr,4,AData)=FALSE then begin ViewAny('eCannot push data at 0x'+IntToHex(BAddr,8)+' [R:TCoreWA.PushD]'); break; end;
 Result:=TRUE;
 until TRUE;
End;

Function TCoreWA.PopD ( Out AData : Cardinal ) : boolean;
Var
  BAddr     : Cardinal;
Begin
 Result:=FALSE;
 ...
 repeat
 BAddr:=GetSp;
 if MioRdX(BAddr,4,AData)=FALSE then begin ViewAny('eCannot pop data at 0x'+IntToHex(BAddr,8)+' [R:TCoreWA.PopD]'); break; end;
 BAddr:=BAddr+4;
 SetSp(BAddr);
 Result:=TRUE;
 until TRUE;
...
Function TCoreWA.GetIp : Cardinal;
Begin
 Result:=FRegMIp.FIp;
End;

Procedure TCoreWA.SetIp ( AData : Cardinal );
Begin
 FRegMIp.FIp:=AData;
End;

Procedure TCoreWA.UpdateIp;
Begin
 SetIp(FRegMIp.FIp+Length(FExec.CodeBin));
End;

Function TCoreWA.ExecInvalid : boolean;
Begin
 ViewAny('eCmd code is invalid [R:TCoreWA.ExecInvalid]');
 Result:=FALSE;
End;

Function TCoreWA.JmpEn ( ACond : byte ) : boolean;
Var
  BSmaller  : byte;
  BFlags    : byte;
Begin
...
Procedure TCoreWA.SetFlagVN_A ( ADataR, ADataU, ADataS : Cardinal; Var AFlags : byte; AMask : byte );
Var
...
Procedure TCoreWA.SetFlagVN_S ( ADataR, ADataU : Cardinal; Var AFlags : byte );
Var
...
Procedure TCoreWA.SignExtA ( Var AData : Cardinal; AWw : byte; ASignExt : boolean );
Var
  BMaskOr   : Cardinal;
  BMaskAnd  : Cardinal;
Begin
...
Procedure TCoreWA.SignExt ( Var ADataU, ADataS : Cardinal; ASignExt : boolean );
Begin
 SignExtA(ADataU,FExec.Subdec.FTarg[rtU].FWw,ASignExt);
 SignExtA(ADataS,FExec.Subdec.FTarg[rtS].FWw,ASignExt);
End;

Function TCoreWA.AluA ( Out ADataR : Cardinal; ADataU, ADataS : Cardinal; ACmd : byte; ASignExt : byte ) : boolean;
Var
  BDataS,
  BDataU,
  BDataR    : Cardinal;
...
Begin
 ...
 case ACmd of
  ...
  15: begin // fdiv
      ...
      ViewAny('eInvalid AC command code [R:TCoreWA.AluA]');
      break;
      end;
 end; // case
...
Function TCoreWA.AluF1 ( Var ADataR : Cardinal; ACmd : byte ) : boolean;
Var
  BDataR    : Cardinal;
  BDataRF   : Single absolute BDataR;
Begin
 ...
 case ACmd of
   ...
   2: begin // round
      ...
      ViewAny('eInvalid AF command code [R:TCoreWA.AluF1]');
      break;
      end;
 end; // case
 Result:=TRUE;
...
Function TCoreWA.AluT ( Out ADataR : Cardinal; ADataU, ADataS : Cardinal; ACmd : byte ) : boolean;
Var
  BMask     : Cardinal;
  BFlags    : byte;
Begin
 ...
 case ACmd of
   ...
   2: begin // bts
      ...
      ViewAny('eInvalid BT command code [R:TCoreWA.AluT]');
      break;
      end;
 end; // case
...
// ARUC   |00      ||  ccc4ww|uuuurrrr|#2/4             | add sub and or xor mul udiv sdiv | (r := u cmd Const)
Function TCoreWA.ExecARUC : boolean;
Var
  BDataS,
  BDataU,
  BDataR    : Cardinal;
...
// BM   |00   10 ||  iii  i|iiiirrrr|                 | mov (bytes) (Unused codes of ARUC (because 4=1(32 bit) cannot be used with byte/word constant)
Function TCoreWA.ExecBM  : boolean;
Begin
...
// ARRS |01   0  ||  ccc ww|ssssrrrr|                 | add sub mov cmp and or test xor r/r of same size (Unused codes of ARUC, because FPU cannot use )
Function TCoreWA.ExecARRS : boolean;
Var
  BDataS,
  BDataU,
  BDataR    : Cardinal;
...
// BC   |01   10 ||  iii  i|iiiirrrr|                 | cmp (bytes)
Function TCoreWA.ExecBC : boolean;
Var
  BDataS,
  BDataU,
  BDataR    : Cardinal;
...
// BA   |01   11 ||  iii  i|iiiirrrr|                 | add (bytes)
Function TCoreWA.ExecBA : boolean;
Var
  BDataS,
  BDataU,
  BDataR    : Cardinal;
...
// MAID |100     ||   cccww|aaaarrrr|                 | rd[reg] RFU wr[reg] RFU rd[reg++] rd[--reg] wr[reg++] wr[--reg] | ccc = {use_inc_dec wr/rd -/+}
Function TCoreWA.ExecMAID : boolean;
Var
  BAddr     : Cardinal;
  BDataA,
  BDataR    : Cardinal;
...
// MARC |1010    ||    c4ww|aaaarrrr|#2/4             | Mem r/w
Function TCoreWA.ExecMARC : boolean;
Var
  BAddr     : Cardinal;
  BDataR    : Cardinal;
Begin
 ...
 if FExec.Subdec.FCccc=0 then // read
  begin
  if MioRdX(BAddr,TargSize(rtR),BDataR)=FALSE then begin ViewAny('eError reading data from address 0x'+IntToHex(BAddr,4)+' [R:TCoreWA.ExecMARC]'); break; end;
  WrReg(rtR,BDataR);
  end
 else // write
  begin
  BDataR:=RdReg(rtR);
  if MioWrX(BAddr,TargSize(rtR),BDataR)=FALSE then begin ViewAny('eError writing data to address 0x'+IntToHex(BAddr,4)+' [R:TCoreWA.ExecMARC]'); break; end;
  end;
...
// ONL  |10110   ||     4cc|uuuuffff|#2/4             | jmp call 2xRFU  (either REG or # but not both, call REG is not possible)
Function TCoreWA.ExecONL : boolean;
Var
  BNewIp        : Cardinal;
Begin
 Result:=FALSE;
 repeat
 if FExec.Subdec.FCccc<>0 then begin ViewAny('eInvalid ONL command code [R:TCoreWA.ExecONL]'); break; end;
...
// IDE  |10111   ||     cww|0iiirrrr|                 | inc dec
Function TCoreWA.ExecIDE : boolean;
Var
  BDataS,
  BDataU,
  BDataR    : Cardinal;
...
// PEX  |10111   ||     cww|1000rrrr|                 | pushzx pushsx
Function TCoreWA.ExecPEX : boolean;
Var
  BDataR    : Cardinal;
Begin
 Result:=FALSE;
...
// AF   |10111   ||     ccc|1001rrrr|                 | itf trunc round 5xRFU
Function TCoreWA.ExecAF : boolean;
Var
  BDataR    : Cardinal;
Begin
 Result:=FALSE;
...
// REM  |10111   ||     cww|1010rrrr|                 | rem RFU
Function TCoreWA.ExecREM : boolean;
Begin
 Result:=FALSE;
 repeat
 if FExec.Subdec.FCccc=0 then
 else begin ViewAny('eInvalid REM command code [R:TCoreWA.ExecREM]'); break; end;
 UpdateIp;
 Result:=TRUE;
 until TRUE;
End;
...
Function TCoreWA.ExecPRCS : boolean;
Begin
 Result:=FALSE;
 repeat
 case FExec.Subdec.FCccc of
    0: begin
       SetIp(GetIp+FExec.Subdec.FConst);
    ...
    1: begin
       ...
       ViewAny('eInvalid PRCS command code [R:TCoreWA.ExecREM]');
       break;
       end;
 end; // case
 Result:=TRUE;
...
Function TCoreWA.ExecSys : boolean;
Begin
 Result:=FALSE;
 repeat
 case FExec.Subdec.FCccc of
    ...
    4: begin // siconf
       ...
       ViewAny('eInvalid SYS command code [R:TCoreWA.ExecNTRE]');
       break;
       end;
 end; // case
 until TRUE;
...
Function TCoreWA.ExecNTRE : boolean;
Begin
 Result:=FALSE;
 repeat
 case FExec.Subdec.FCccc of
    ...
    7: begin // setfl
       ...
       ViewAny('eInvalid NTRE command code [R:TCoreWA.ExecNTRE]');
       break;
       end;
 end; // case
 Result:=TRUE;
...
// PPL  |10111   ||     ccc|10110011|#2               | pushl popl 6xRFU
Function TCoreWA.ExecPPL : boolean;
Var
  BRegIdx   : Integer;
  BData     : Cardinal;
  BUpdateIp : boolean;
Begin
 ...
 case FExec.Subdec.FCccc of
    ...
    5: begin // pushl/enter
       ...
       ViewAny('eInvalid PPL command code [R:TCoreWA.ExecPPL]');
       break;
       end;
 end; // case
...
// RFU  |10111   ||        |        |
Function TCoreWA.ExecRFU : boolean;
Begin
 Result:=FALSE;
 ViewAny('eTrying to execute RFU command [R:TCoreWA.ExecRFU]');
End;

// ONS  |110     ||   djjjj|jjjjffff|                 | sjmp jbe jc jnc jz jnz ja RFU jg jge js jse jn jv jnn jnv
Function TCoreWA.ExecONS : boolean;
Var
  BNewIp    : Cardinal;
Begin
 Result:=FALSE;
...
Function TCoreWA.ExecFRUC : boolean;
Var
  BDataS,
  BDataU,
  BDataR    : Cardinal;
...
// FRRS |11101   ||     ccc|ssssrrrr|                 | fadd fsub fmul fdiv 4xRFU | (r = r cmd s)
Function TCoreWA.ExecFRRS : boolean;
Var
  BDataS,
  BDataU,
  BDataR    : Cardinal;
...
// BTR  |11110   ||     icc|iiiirrrr|                 | bt btr bts btx (test is always 32-bit: if reg=AH[bit_1], then iiiiiii=9)
Function TCoreWA.ExecBTR : boolean;
Var
  BDataS,
  BDataU,
  BDataR    : Cardinal;
...
// BTM  |111110  ||      cc|aaaa4iii|#2/4             | bt btr bts btx
Function TCoreWA.ExecBTM : boolean;
Var
  BAddr     : Cardinal;
  BDataU,
  BDataR    : Cardinal;
...
// ARUI   |11111100||        |uuuurrrr|ccccwwww|iiiiiiii| addex subex andex orex xorex mulex udivex sdivex fadd fsub fmul fdiv shl shr asr rol | (r = u cmd i)
Function TCoreWA.ExecARUI : boolean;
Var
  BDataS,
  BDataU,
  BDataR    : Cardinal;
...
Function TCoreWA.ExecARUS : boolean;
Var
  BDataS,
  BDataU,
  BDataR    : Cardinal;
